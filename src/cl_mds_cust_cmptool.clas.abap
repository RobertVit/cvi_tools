class CL_MDS_CUST_CMPTOOL definition
  public
...
public section.
*"* public components of class CL_MDS_CUST_CMPTOOL
*"* do not include other source files here!!!
  ...
  methods IF_MDS_CMPTOOL~FINALIZE
    redefinition .
protected section.
*"* protected components of class CL_MDS_CUST_CMPTOOL
*"* do not include other source files here!!!

  constants GC_PRIMARY_KEY type FIELDNAME value 'KUNNR'. "#EC NOTEXT
  constants GC_SOURCE_OBJECT type MDS_CMPT_CTRL_OBJ_SOURCE value 'CUSTOMER'. "#EC NOTEXT
  class-data GR_INSTANCE type ref to CL_MDS_CUST_CMPTOOL .
private section.
*"* private components of class CL_MDS_CUST_CMPTOOL
*"* do not include other source files here!!!
...
CLASS CL_MDS_CUST_CMPTOOL IMPLEMENTATION.
...
METHOD compare_source_target.
  ...
  DATA: lr_instance TYPE REF TO cl_mds_cmpt_compare,
        ...
        ls_source TYPE cmds_ei_main,
        ls_target TYPE cmds_ei_main,
...
* Compare the source and target structures
  CALL METHOD lr_instance->compare_customer_as_source
    EXPORTING
...
METHOD extract_data.
  ...
  DATA: lt_id_list     TYPE   mds_cmpt_source_target_t,
        ...
        lt_messages    TYPE   mds_ctrls_error_objects,
        ls_source_data TYPE   cmds_ei_main,
        ls_target_data TYPE   bus_ei_main,
        ls_cust_id     TYPE   mds_cmpt_object,
        lt_cust_ids    TYPE   mds_cmpt_object_t,
...
* Object specific error messages
  ...
  LOOP AT lt_id_list INTO ls_id_list.
    ls_cust_id = ls_id_list-source_object.
    ls_bp_id = ls_id_list-target_object.
    APPEND ls_cust_id TO lt_cust_ids.
    APPEND ls_bp_id TO lt_bp_ids.
  ENDLOOP.

*Extract source data - Customer
  CALL METHOD cl_mds_cmptool_extract=>extract_cust_data
    EXPORTING
      is_control  = is_control
      it_id_list  = lt_cust_ids
    IMPORTING
...
* Object specific error messages
  ...
  CREATE DATA lr_source TYPE cmds_ei_main.
...
DATA: lt_kunnr                   TYPE TABLE OF cmds_customer_number,
      lt_part_of_site            TYPE cmds_customer_numbers_s_t,
      ls_cmds_customer_number    TYPE cmds_customer_number,
...
*      MESSAGE x006 WITH 'CX_MDS_EXTRACTOR' 'extract_id' is_control-source-compare_class.
  ...
  lt_kunnr = lt_object_id.

* Filter the customers, which are part of a retail
  ...
  IF NOT lt_part_of_site[] IS INITIAL.
    ...
    LOOP AT lt_part_of_site INTO ls_cmds_customer_number.
      READ TABLE lt_kunnr
                WITH KEY kunnr = ls_cmds_customer_number-kunnr
           BINARY SEARCH
           TRANSPORTING NO FIELDS.
      IF sy-subrc = 0.
        DELETE lt_kunnr INDEX sy-tabix.
      ENDIF.
    ENDLOOP.
  ENDIF.

  et_object_id = lt_kunnr.
  et_messages = ls_error_objects.
  ...
  CASE iv_target_object.
    WHEN business_partner.
       SELECT partner_guid customer FROM cvi_cust_link
                      ...
                      WHERE customer = lt_query_ids-source_id.
       IF NOT lt_ids4_guid IS INITIAL.
           SORT lt_ids4_guid by partner_guid.
...
DATA : lv_ktokd                TYPE ktokd,
       ...
       lt_accountgroups        TYPE TABLE OF kna1-ktokd,
       lt_valid_accountgroups  TYPE TABLE OF kna1-ktokd.

*Step 1: Get all valid account groups from account group <-> grouping mapping
  SELECT account_group FROM cvic_cust_to_bp1 INTO TABLE lt_accountgroups.
  IF sy-subrc <> 0.
    CALL METHOD fsbp_generic_services=>new_message
      EXPORTING
        i_class_id = 'CVI_TOOLS_SUPPORT'
...
*Step 2: Get all valid account groups from account group <-> role mapping
  SELECT account_group FROM cvic_cust_to_bp2 INTO TABLE lt_accountgroups.
  IF sy-subrc <> 0.
    CALL METHOD fsbp_generic_services=>new_message
      EXPORTING
        i_class_id = 'CVI_TOOLS_SUPPORT'
...
*   get the object's account group and check if its valid for sync
    SELECT SINGLE ktokd FROM kna1 INTO lv_ktokd WHERE kunnr = ls_object_id.
    READ TABLE lt_valid_accountgroups FROM lv_ktokd
                                      TRANSPORTING NO FIELDS.
    IF sy-subrc  = 0.
...
* Map business partners to customers -> target to Source
      CALL METHOD lr_ref_mapper->map_bps_to_customers
        EXPORTING
          i_partners  = ls_bus_ei_main
        IMPORTING
          e_customers = ls_customers-customers
          e_errors    = ls_error.
...
* Mapped target structure
  CREATE DATA ls_mapped_data TYPE cmds_ei_main.
  ASSIGN ls_mapped_data->* TO <ls_cust>.
  <ls_cust> = ls_customers.
