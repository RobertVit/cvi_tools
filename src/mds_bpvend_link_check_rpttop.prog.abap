*&---------------------------------------------------------------------*
*& Include MDS_BPVEND_LINK_CHECK_RPTTOP     Module Pool      MDS_BPVEND_LINK_CHECK_REPORT
*&
*&---------------------------------------------------------------------*
PROGRAM  MDS_BPVEND_LINK_CHECK_REPORT.

* Global type
types:  begin of gty_bp_vendor_ids_alv,
         partner  type bu_partner,
         vendor   type lifnr,
        end of gty_bp_vendor_ids_alv.
...
DATA : gc_x                      TYPE boole-boole VALUE 'X',
       ...
       gs_entries_to_delete_alv  TYPE gty_bp_vendor_ids_alv,
       gt_entries_to_delete_alv  TYPE table of gty_bp_vendor_ids_alv.
...
OPEN CURSOR db_vendor FOR
     select PARTNER_GUID VENDOR from cvi_vend_link.             "#EC CI_NOWHERE
  ...
  ELSE.
    LOOP AT lt_bp_vendor_ids INTO ls_bp_vendor_ids.
      ...
      select single partner_guid FROM  but000
        ...
        IF sy-subrc <> 0.
          ls_bp_vendor_ids-del_flag = gc_x.
        ELSE.
          clear lv_lifnr.
          select single lifnr FROM  lfa1
           ...
           ENDIF.
        ENDIF.
        IF ls_bp_vendor_ids-del_flag = gc_x.
...
* Set up the data to display what has changed.
  LOOP AT lt_entries_to_delete INTO ls_entries_to_delete.
    ...
    gs_entries_to_delete_alv-vendor = ls_entries_to_delete-vendor.
